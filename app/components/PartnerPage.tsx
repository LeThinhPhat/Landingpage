...
// ✅ Cách 1: GIỮ NGUYÊN URL online như bạn đang dùng
const PARTNERS = [
  {
    name: "UPS",
...
const PartnerCard = ({ name, logo, url }: Partner) => {
  const isDataUrl = logo.startsWith("data:image");
  ...
  return (
    ...
    >
      {/* 
        ✅ next/image cần width/height.
        - Với data URL (base64) vẫn dùng Image được.
        - Với SVG/PNG online: dùng Image (nhớ config domain).
      */}
      <Image
        src={logo}
        alt={name}
        width={180}
        height={56}
        className="max-h-12 sm:max-h-14 w-auto object-contain transition duration-300"
        unoptimized={isDataUrl} // base64 để an toàn, khỏi lỗi optimize
      />
    </Link>
  );
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <div className="text-center mb-10">
          ...
          <div className="mt-3 flex justify-center">
            <span className="h-1 w-36 bg-yellow-400 rounded-full" />
          </div>
        </div>
